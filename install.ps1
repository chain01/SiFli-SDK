...
$TARGETS = (python "$SIFLI_SDK_PATH/tools/install_util.py" extract targets "$args")
...
$proces_tools = Start-Process -Wait -PassThru -NoNewWindow -FilePath "python" -Args "`"$SIFLI_SDK_PATH/tools/sifli_sdk_tools.py`" --sifli-sdk-path ${SIFLI_SDK_PATH} install --targets=${TARGETS}"
