...
# sifli_sdk_tools_path variable prior to running this tool.
#
# Users do not need to interact with this script directly. In SDK root directory,
...
SIFLI_SDK_ENV_FILE = 'sifli-sdk-env.json'
TOOLS_FILE_VERSION = 2
SIFLI_SDK_TOOLS_PATH_DEFAULT = os.path.join('~', '.sifli')
UNKNOWN_VERSION = 'unknown'
SUBST_TOOL_PATH_REGEX = re.compile(r'\${TOOL_PATH}')
VERSION_REGEX_REPLACE_DEFAULT = r'\1'
SDK_MAINTAINER = os.environ.get('SDK_MAINTAINER') or False
TODO_MESSAGE = 'TODO'
...
class GlobalVarsStore:
    ...
    _string_properties = ['sifli_sdk_path', 'sifli_sdk_tools_path', 'tools_json']
...
class Platforms:
    ...
    # List of platforms that are not supported by SiFli-SDK
...
class SiFliSDKToolDownload(object):
    """
...
class SiFliSDKToolVersion(object):
    ...
    def get_download_for_platform(self, platform_name: Optional[str]) -> Optional[SiFliSDKToolDownload]:
        """
...
SiFliSDKToolOptions = namedtuple('SiFliSDKToolOptions', [
    'version_cmd',
    'version_regex',
...
class SiFliSDKTool(object):
    ...
    def __init__(self, name: str,
        ...
        self.options = SiFliSDKToolOptions(version_cmd, version_regex, version_regex_replace, is_executable,
    ...
    def _update_current_options(self) -> None:
        ...
        """
        self._current_options = SiFliSDKToolOptions(*self.options)
    ...
    def drop_versions(self) -> None:
        ...
        """
        self.versions: Dict[str, SiFliSDKToolVersion] = OrderedDict()
    ...
    def get_path(self) -> str:
        ...
        """
        return os.path.join(g.sifli_sdk_tools_path, 'tools', self.name)
    ...
    def is_supported_for_any_of_targets(self, targets: List[str]) -> bool:
        ...
        return bool(any(item in targets for item in supported_targets) or supported_targets == ['all'])
    ...
    def find_installed_versions(self) -> None:
        """
        Checks whether the tool can be found in PATH and in GlobalVarsStore.sifli_sdk_tools_path.
        ...
        # Now check all the versions installed in GlobalVarsStore.sifli_sdk_tools_path
    ...
    def download(self, version: str) -> None:
        ...
        local_path = os.path.join(g.sifli_sdk_tools_path, 'dist', archive_name)
    ...
    def install(self, version: str) -> None:
        ...
        archive_path = os.path.join(g.sifli_sdk_tools_path, 'dist', archive_name)
    ...
    @staticmethod
    def check_download_file(download_obj: SiFliSDKToolDownload, local_path: str) -> bool:
        """
    ...
    def from_json(cls, tool_dict: Dict[str, Union[str, List[str], Dict[str, str]]]) -> 'SiFliSDKTool':
        ...
        # Create the object
        tool_obj: 'SiFliSDKTool' = cls(tool_name, description, install, info_url, license,  # type: ignore
...
class SiFliSDKEnvEncoder(JSONEncoder):
    """
    SiFliSDKEnvEncoder is used for encoding SiFliSDKEnv, SiFliSDKRecord, SelectedSiFliSDKRecord classes to JSON in readable format. Not as (__main__.SiFliSDKRecord object at '0x7fcxx').
...
class SiFliSDKRecord:
    """
    SiFliSDKRecord represents one record of installed SiFli-SDK on system.
    Contains:
        * version - actual version of SiFli-SDK (example '2.0')
        * path - absolute path to the SiFli-SDK
        * features - features using SiFli-SDK
        * targets - SiFli chips for which are installed needed toolchains (example ['sf32lb52' , 'sf32lb56'])
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, SiFliSDKRecord):
            return False
    ...
    @classmethod
    def get_active_sifli_sdk_record(cls) -> 'SiFliSDKRecord':
        sifli_sdk_record_obj = cls()
        sifli_sdk_record_obj.version = get_sifli_sdk_version()
        sifli_sdk_record_obj.path = g.sifli_sdk_path
        return sifli_sdk_record_obj

    @classmethod
    def get_sifli_sdk_record_from_dict(cls, record_dict: Dict[str, Any]) -> 'SiFliSDKRecord':
        sifli_sdk_record_obj = cls()
        try:
            sifli_sdk_record_obj.version = record_dict['version']
            sifli_sdk_record_obj.path = record_dict['path']
        except KeyError:
        ...
        sifli_sdk_record_obj.update_features(record_dict.get('features', []))
        sifli_sdk_record_obj.extend_targets(record_dict.get('targets', []))
...
class SiFliSDKEnv:
    """
    SiFliSDKEnv represents SiFli-SDK Environments installed on system and is responsible for loading and saving structured data.
    All information is saved and loaded from SIFLI_SDK_ENV_FILE.
    Contains:
    ...
    def __iter__(self):  # type: ignore
        yield from {
            'sifliSdkInstalled': self.sifli_sdk_installed,
        }.items()

    def __str__(self) -> str:
        return json.dumps(dict(self), cls=SiFliSDKEnvEncoder, ensure_ascii=False, indent=4)  # type: ignore
    ...
    def save(self) -> None:
        ...
        if self.get_active_sifli_sdk_record() != self.get_sifli_sdk_env().get_active_sifli_sdk_record():
            sifli_sdk_env_file_path = os.path.join(g.sifli_sdk_tools_path, SIFLI_SDK_ENV_FILE)
            try:
                if g.sifli_sdk_tools_path:  # mypy fix for Optional[str] in the next call
                    # the directory doesn't exist if this is run on a clean system the first time
                    mkdir_p(g.sifli_sdk_tools_path)
                with open(sifli_sdk_env_file_path, 'w', encoding='utf-8') as w:
                    info(f'Updating {sifli_sdk_env_file_path}')
                    json.dump(dict(self), w, cls=SiFliSDKEnvEncoder, ensure_ascii=False, indent=4)  # type: ignore
            except (IOError, OSError):
                if not os.access(g.sifli_sdk_tools_path, os.W_OK):
                    raise OSError(f'sifli_sdk_tools_path {g.sifli_sdk_tools_path} is not accessible to write. '
                                  'Required changes have not been saved')
                raise OSError(f'File {sifli_sdk_env_file_path} is not accessible to write or corrupted. Required changes have not been saved')

    def get_active_sifli_sdk_record(self) -> SiFliSDKRecord:
        return self.sifli_sdk_installed[active_repo_id()]

    @classmethod
    def get_sifli_sdk_env(cls) -> 'SiFliSDKEnv':
        """
        SDKEnv class is used to process SIFLI_SDK_ENV_FILE file. The constructor is therefore called only in this method that loads the file and checks its contents.
        """
        sifli_sdk_env_obj = cls()
        try:
            sifli_sdk_env_file_path = os.path.join(g.sifli_sdk_tools_path, SIFLI_SDK_ENV_FILE)
            with open(sifli_sdk_env_file_path, 'r', encoding='utf-8') as sifli_sdk_env_file:
                sifli_sdk_env_json = json.load(sifli_sdk_env_file)

                try:
                    sifli_sdk_installed = sifli_sdk_env_json['sifliSdkInstalled']
                except KeyError:
                    # If no SiFli-SDK record is found in loaded file, do not update and keep default value from constructor
                    pass
                else:
...
class ENVState:
    """
    ENVState is used to handle SDK global variables that are set in environment and need to be removed when switching between SiFli-SDK versions in opened shell.
    Every opened shell/terminal has it's own temporary file to store these variables.
    The temporary file's name is generated automatically with suffix 'sifli_sdk_ + opened shell ID'. Path to this tmp file is stored as env global variable (env_key).
    ...
    * sifli_sdk_variables - loaded SiFli-SDK variables from file
    """
    env_key = 'SIFLI_SDK_DEACTIVATE_FILE_PATH'
    deactivate_file_path = os.environ.get(env_key, '')

    def __init__(self) -> None:
        self.sifli_sdk_variables: Dict[str, Any] = {}
        ...
        if cls.deactivate_file_path:
            try:
                with open(cls.deactivate_file_path, 'r', encoding='utf-8') as fp:
                    env_state_obj.sifli_sdk_variables = json.load(fp)
            except (IOError, OSError, ValueError):
                pass
    ...
    def save(self) -> str:
        try:
            if self.deactivate_file_path and os.path.basename(self.deactivate_file_path).endswith(f'sifli_sdk_{str(os.getppid())}'):
                ...
                with open(self.deactivate_file_path, 'w', encoding='utf-8') as w:
                    json.dump(self.sifli_sdk_variables, w, ensure_ascii=False, indent=4)  # type: ignore
            else:
                with tempfile.NamedTemporaryFile(delete=False, suffix=f'sifli_sdk_{str(os.getppid())}') as fp:
...
def load_tools_info() -> Dict[str, SiFliSDKTool]:
    """
...
def parse_tools_info_json(tools_info):  # type: ignore
    ...
    for tool_dict in tools_array:
        tool = SiFliSDKTool.from_json(tool_dict)
        tools_dict[tool.name] = tool

    return tools_dict


def dump_tools_json(tools_info: Optional[Dict[str, SiFliSDKTool]]):  # type: ignore
    """
...
def get_sifli_sdk_version() -> str:
    """
    Return SiFli-SDK version.
    """
    sifli_sdk_version: Optional[str] = None

    version_file_path = os.path.join(g.sifli_sdk_path, 'version.txt')
    if os.path.exists(version_file_path):
        with open(version_file_path, 'r', encoding='utf-8') as version_file:
            sifli_sdk_version_str = version_file.read()

        match = re.match(r'^v([0-9]+\.[0-9]+).*', sifli_sdk_version_str)
        if match:
            sifli_sdk_version = match.group(1)

    if sifli_sdk_version is None:
        fatal('Failed to determine SiFli-SDK version')
        raise SystemExit(1)
        # try:
        #     with open(os.path.join(g.sifli_sdk_path, 'components', 'esp_common', 'include', 'esp_idf_version.h'), encoding='utf-8') as f:
        #         m = re.search(r'^#define\s+ESP_IDF_VERSION_MAJOR\s+(\d+).+?^#define\s+ESP_IDF_VERSION_MINOR\s+(\d+)',
        #                       f.read(), re.DOTALL | re.MULTILINE)
        #         if m:
        #             sifli_sdk_version = '.'.join((m.group(1), m.group(2)))
        #         else:
...
def get_python_env_path() -> Tuple[str, str, str, str]:
    ...
    sifli_sdk_python_env_path = os.getenv('SIFLI_SDK_PYTHON_ENV_PATH') or os.path.join(g.sifli_sdk_tools_path,
                                                                           'python_env',
                                                                           PYTHON_VENV_DIR_TEMPLATE.format(sifli_sdk_version,
                                                                                                           PYTHON_VER_MAJOR_MINOR))

    python_exe, subdir = get_python_exe_and_subdir()
    sifli_sdk_python_export_path = os.path.join(sifli_sdk_python_env_path, subdir)
    virtualenv_python = os.path.join(sifli_sdk_python_export_path, python_exe)

    return sifli_sdk_python_env_path, sifli_sdk_python_export_path, virtualenv_python, sifli_sdk_version
...
def expand_tools_arg(tools_spec: List[str], overall_tools: OrderedDict, targets: List[str]) -> List[str]:
    ...
    if 'required' in tools_spec:
        tools.extend([k for k, v in overall_tools.items() if v.get_install_type() == SiFliSDKTool.INSTALL_ALWAYS and k not in tools])

    elif 'all' in tools_spec:
        tools.extend([k for k, v in overall_tools.items() if v.get_install_type() != SiFliSDKTool.INSTALL_NEVER and k not in tools])
...
def filter_tools_info(sifli_sdk_env_obj: SiFliSDKEnv, tools_info: Dict[str, SiFliSDKTool]) -> Dict[str,SiFliSDKTool]:
    """
    ...
    if not targets:
        ...
        filtered_tools_spec = {k:v for k, v in tools_info.items() if
                               (v.get_install_type() == SiFliSDKTool.INSTALL_ALWAYS or v.get_install_type() == SiFliSDKTool.INSTALL_ON_REQUEST) and
...
def add_variables_to_deactivate_file(args: List[str], new_sifli_sdk_vars:Dict[str, Any]) -> str:
    """
    Add SDK global variables that need to be removed when the active sifli-sdk environment is deactivated.
    """
    if 'PATH' in new_sifli_sdk_vars:
        new_sifli_sdk_vars['PATH'] = new_sifli_sdk_vars['PATH'].split(':')[:-1]  # PATH is stored as list of sub-paths without '$PATH'

    new_sifli_sdk_vars['PATH'] = new_sifli_sdk_vars.get('PATH', [])
    args_add_paths_extras = vars(args).get('add_paths_extras')  # remove mypy error with args
    new_sifli_sdk_vars['PATH'] = new_sifli_sdk_vars['PATH'] + args_add_paths_extras.split(':') if args_add_paths_extras else new_sifli_sdk_vars['PATH']
...
def print_deactivate_statement(args: List[str]) -> None:
    ...
    """
    env_state_obj = ENVState.get_env_state()
    if not env_state_obj.sifli_sdk_variables:
        warn('No SiFli-SDK variables to remove from environment found. Deactivation of previous sifli-sdk version was not successful.')
        return
    unset_vars = env_state_obj.sifli_sdk_variables
    ...
    env_state_obj.sifli_sdk_variables.clear()
    env_state_obj.save()
    return
...
def active_repo_id() -> str:
    ...
    """
    try:
        # g.sifli_sdk_path is forcefully casted to str just to make type linters happy
        return f'{str(g.sifli_sdk_path)}-v{get_sifli_sdk_version()}'
    except ReferenceError:
        return f'UNKNOWN_PATH-v{get_sifli_sdk_version()}'
...
def list_default(args):  # type: ignore
    ...
    for name, tool in tools_info.items():
        if tool.get_install_type() == SiFliSDKTool.INSTALL_NEVER:
            continue
        optional_str = ' (optional)' if tool.get_install_type() == SiFliSDKTool.INSTALL_ON_REQUEST else ''
...
def action_check(args):  # type: ignore
    ...
    for name, tool in tools_info.items():
        ...
        for version in tool.versions_installed:
            ...
            tool_found_somewhere = True
        if not tool_found_somewhere and tool.get_install_type() == SiFliSDKTool.INSTALL_ALWAYS:
...
def process_tool(
    ...
    if not tool.versions_installed:
        if tool.get_install_type() == SiFliSDKTool.INSTALL_ALWAYS:
            if os.getenv('SIFLI_SDK_SKIP_TOOLS_CHECK', '0') == '1':
                warn(f'Tool {tool_name} is not installed and SIFLI_SDK_SKIP_TOOLS_CHECK is set. '
...
def get_sifli_sdk_download_url_apply_mirrors(args: Any = None, download_url: str = SIFLI_SDK_DL_URL) -> str:
    """
    Returns URL for SiFli-SDK download with applied mirrors if available.
    If original URL pointed to Github and SIFLI_SDK_GITHUB_ASSETS is set, change the source to SiFli's download servers.
    """
...
def apply_mirror_prefix_map(args: Any, idf_download_url: str) -> str:
    ...
    if SDK_MAINTAINER and args and args.mirror_prefix_map:
        if mirror_prefix_map:
            warn('Both SIFLI_SDK_MIRROR_PREFIX_MAP environment variable and --mirror-prefix-map flag are specified, '
...
def apply_github_assets_option(sifli_sdk_download_url: str) -> str:
    """
    Rewrite URL for given sifli_sdk_download_url if the download URL is an https://github.com/ URL and the variable
    SIFLI_SDK_GITHUB_ASSETS is set. The github.com part of the URL will be replaced.
    """
    new_url = sifli_sdk_download_url
    github_assets = os.environ.get('SIFLI_SDK_GITHUB_ASSETS', '').strip()
    if not github_assets:
        # no SIFLI_SDK_GITHUB_ASSETS or variable exists but is empty
        return new_url

    # check no URL qualifier in the mirror URL
    if '://' in github_assets:
        fatal("SIFLI_SDK_GITHUB_ASSETS shouldn't include any URL qualifier, https:// is assumed")
        raise SystemExit(1)

    # Strip any trailing / from the mirror URL
    github_assets = github_assets.rstrip('/')

    new_url = re.sub(r'^https://github.com/', f'https://{github_assets}/', sifli_sdk_download_url)
    if new_url != sifli_sdk_download_url:
        info(f'Using GitHub assets mirror for URL: {sifli_sdk_download_url} => {new_url}')
    return new_url
...
def action_install(args):  # type: ignore
    """
    Saves current SiFli-SDK environment and for every tool in tools_spec, installs the tool from the archive downloaded before, if possible.
...
def get_wheels_dir() -> Optional[str]:
    """
    Gets path for sifli-sdk-python-wheels package.
    """
    tools_info = load_tools_info()
    wheels_package_name = 'sifli-sdk-python-wheels'
...
class ChecksumFileParser():
    ...
    def __init__(self, filename_prefix: str, url: str) -> None:
        ...
        sha256_file_tmp = os.path.join(g.sifli_sdk_tools_path, 'tools', 'add-version.sha256.tmp')
...
def action_add_version(args: Any) -> None:
    ...
    if not tool_obj:
        ...
        tool_obj = SiFliSDKTool(tool_name, TODO_MESSAGE, SiFliSDKTool.INSTALL_ALWAYS,
    ...
    if not args.output:
        args.output = os.path.join(g.sifli_sdk_path, TOOLS_FILE_NEW)  # type: ignore
...
def action_rewrite(args):  # type: ignore
    ...
    json_str = dump_tools_json(tools_info)
    if not args.output:
        args.output = os.path.join(g.sifli_sdk_path, TOOLS_FILE_NEW)
    with open(args.output, 'w', encoding='utf-8') as f:
        f.write(json_str)
        f.write('\n')
    info(f'Wrote output to {args.output}')
...
def action_uninstall(args: Any) -> None:
    ...
    """
    tools_info = load_tools_info()
    tools_path = os.path.join(g.sifli_sdk_tools_path, 'tools')
    dist_path = os.path.join(g.sifli_sdk_tools_path, 'dist')
...
def action_validate(args):  # type: ignore
    ...
    with open(os.path.join(g.sifli_sdk_path, TOOLS_FILE), 'r', encoding='utf-8') as tools_file:
        tools_json = json.load(tools_file)

    with open(os.path.join(g.sifli_sdk_path, TOOLS_SCHEMA_FILE), 'r', encoding='utf-8') as schema_file:
...
""".rstrip().format(name=tool_name,
        ...
        for platform_name in sorted(tool_obj.get_supported_platforms()):
            ...
            if install_type == SiFliSDKTool.INSTALL_NEVER:
                continue
            elif install_type == SiFliSDKTool.INSTALL_ALWAYS:
                install_type_str = 'required'
            elif install_type == SiFliSDKTool.INSTALL_ON_REQUEST:
...
def main(argv: List[str]) -> None:
    ...
    parser.add_argument('--sifli-sdk-path', help='SiFli SDK path to use')
    ...
    if sys.version_info.major == 2:
        try:
            g.sifli_sdk_tools_path.decode('ascii')  # type: ignore
        except UnicodeDecodeError:
            fatal(f'sifli_sdk_tools_path contains non-ASCII characters: {g.sifli_sdk_tools_path}'
                  '\nThis is not supported yet with Python 2. '
                  'Please set sifli_sdk_tools_path to a directory with an ASCII name, or switch to Python 3.')
            raise SystemExit(1)
    ...
    if args.tools_json:
        ...
        g.tools_json = os.path.join(g.sifli_sdk_path, TOOLS_FILE)
